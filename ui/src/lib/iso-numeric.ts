/** ISO 3166-1 numeric keyed by alpha-2 (world-atlas country ids). */
const NUM: Record<string, string> = {
  ...
  VN: '704', VU: '548', WF: '876', WS: '882', XK: '983', YE: '887', YT: '175', ZA: '710',
  ZM: '894', ZW: '716',
}

const BY_NUM: Record<string, string> = {}
for (const [cc, n] of Object.entries(NUM)) {
  BY_NUM[String(Number(n))] = cc
  BY_NUM[n] = cc
}

export function iso2FromAtlasId(id: string | number | undefined): string {
  if (id == null || id === '') return ''
  return BY_NUM[String(id)] || BY_NUM[String(Number(id))] || ''
}
